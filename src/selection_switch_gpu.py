...
PRE_BUDGET_STOP_EVALUATION_CODE = "b376059db62ab1bf877768c6e55a51ada39fc2670083934e48a0b0cfdf62afa8"
# Exact 9be50a8 shared runtime, before pre-meter MBPP branch quarantine.
PRE_MBPP_BRANCH_QUARANTINE_CODE = "b06f22b68347f8cce40e8ef57f538311d0d68f2b10bf9fb7d3d59dc4eb49d16b"
# Exact runtime before worker-spawn EAGAIN was distinguished from a peer lease.
PRE_CURVE_SPAWN_CODE = "f71893c62111c5e9d62e2d8d347df151c77233de74c1794153ce5a52efdd754b"
...
PUBLICATION_PATCH_HASHES = {
    "src/net_gain_gate_gpu.py": {"f4604802211d5bac9f7c759e1a199957883d69c04fa7eb6b15f41e58d09637a3",
                                 ...
                                 "99f998677624aba5e955507ed62432b0e4b28e56c44ed2b733dd1964eea65b43"},
...
def bind_reviewed_runtime_receipt(path, receipt, predecessors):
    """Keep prior receipt bytes; accept only an exact reviewed runtime map."""
    if path.exists():
        previous = core.read(path)
        previous_code = previous.get("runtime_code_hashes") if isinstance(previous, dict) else None
        if (previous != receipt and
                (previous != {**receipt, "runtime_code_hashes": previous_code}
                 or core.fingerprint(previous_code) not in predecessors)):
...
def manifest(root):
    ...
    if p["code_hashes"] != current and core.fingerprint(p["code_hashes"]) != PRE_CURVE_SPAWN_CODE:
                    ...
                    if quality_path.exists():
                        ...
                        if (previous != quality_receipt and
                                (previous != {**quality_receipt, "runtime_code_hashes": previous_code}
                                 or core.fingerprint(previous_code) not in {PRE_MATCHED_CODE, PRE_MATCHED_FIX_CODE, PRE_SCORING_LABEL_CODE, PRE_CURVE_LEDGER_CODE, PRE_NODE_ID_CODE, PRE_CURVE_WAIT_CODE, PRE_PUBLICATION_CODE, PRE_ALLOCATION_GUARD_CODE, PRE_RESUME_PRESERVATION_CODE, PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                    ...
                    if label_path.exists():
                        ...
                        if (previous != label_receipt and
                                (previous != {**label_receipt, "runtime_code_hashes": previous_code}
                                 or core.fingerprint(previous_code) not in {PRE_CURVE_LEDGER_CODE, PRE_NODE_ID_CODE, PRE_CURVE_WAIT_CODE, PRE_PUBLICATION_CODE, PRE_ALLOCATION_GUARD_CODE, PRE_RESUME_PRESERVATION_CODE, PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                    ...
                    if ledger_path.exists():
                        ...
                        if (previous != ledger_receipt and
                                (previous != {**ledger_receipt, "runtime_code_hashes": previous_code}
                                 or core.fingerprint(previous_code) not in {PRE_NODE_ID_CODE, PRE_CURVE_WAIT_CODE, PRE_PUBLICATION_CODE, PRE_ALLOCATION_GUARD_CODE, PRE_RESUME_PRESERVATION_CODE, PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                    ...
                    if node_id_path.exists():
                        ...
                        if (previous != node_id_receipt and
                                (previous != {**node_id_receipt, "runtime_code_hashes": previous_code}
                                 or core.fingerprint(previous_code) not in {PRE_CURVE_WAIT_CODE, PRE_PUBLICATION_CODE, PRE_ALLOCATION_GUARD_CODE, PRE_RESUME_PRESERVATION_CODE, PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                    ...
                    if wait_path.exists():
                        ...
                        if (previous != wait_receipt and
                                (previous != {**wait_receipt, "runtime_code_hashes": previous_code}
                                 or core.fingerprint(previous_code) not in {PRE_PUBLICATION_CODE, PRE_ALLOCATION_GUARD_CODE, PRE_RESUME_PRESERVATION_CODE, PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                        ...
                        if publication_path.exists():
                            ...
                            if (previous != publication_receipt and
                                    (previous != {**publication_receipt, "runtime_code_hashes": previous_code}
                                     or core.fingerprint(previous_code) not in {PRE_ALLOCATION_GUARD_CODE, PRE_RESUME_PRESERVATION_CODE, PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                            ...
                            if allocation_path.exists():
                                ...
                                if (previous != allocation_receipt and
                                        (previous != {**allocation_receipt, "runtime_code_hashes": previous_code}
                                         or core.fingerprint(previous_code) not in {PRE_RESUME_PRESERVATION_CODE, PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                            ...
                            if core.fingerprint(current) != PRE_RESUME_PRESERVATION_CODE:
                                ...
                                if resume_path.exists():
                                    ...
                                    if (previous != resume_receipt and
                                            (previous != {**resume_receipt, "runtime_code_hashes": previous_code}
                                             or core.fingerprint(previous_code) not in {PRE_SAVED_POLICY_RECOVERY_CODE, PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                                ...
                                if core.fingerprint(current) != PRE_SAVED_POLICY_RECOVERY_CODE:
                                    ...
                                    if saved_path.exists():
                                        ...
                                        if (previous != saved_receipt and
                                                (previous != {**saved_receipt, "runtime_code_hashes": previous_code}
                                                 or core.fingerprint(previous_code) not in {PRE_CHECKPOINT_RETENTION_CODE, PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})):
                                    ...
                                    if core.fingerprint(current) != PRE_CHECKPOINT_RETENTION_CODE:
                                        retention_path = root / "checkpoint-retention-runtime.json"
                                        bind_reviewed_runtime_receipt(retention_path, {
                                        ...
                                        }, {PRE_BUDGET_STOP_EVALUATION_CODE, PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})
                                        if core.fingerprint(current) != PRE_BUDGET_STOP_EVALUATION_CODE:
                                            bind_reviewed_runtime_receipt(root / "budget-stop-evaluation-runtime.json", {
                                                "schema": "selection-switch-budget-stop-evaluation-runtime/v1",
                                                "switch_sha256": base.digest(root / "switch.json"),
                                                "checkpoint_retention_runtime_sha256": base.digest(retention_path),
                                                "runtime_code_hashes": current,
                                                "change": "validate completed policies on CPU before evaluation resume without charging deployment verification again",
                                                "cost_policy": "preserve all recorded costs, policies, results, choices and budgets; evaluation remains reporting-only",
                                            }, {PRE_MBPP_BRANCH_QUARANTINE_CODE, PRE_CURVE_SPAWN_CODE})
    # This helper is operational code shipped in the immutable runtime snapshot,
    # not an addition to the historical scientific CODE map. Bind it even when
    # switch.json was frozen with the current map; subsequent edits must not be
    # silently accepted by an existing run.
    if core.fingerprint(current) not in PRIOR_RUNTIME_CODES:
        with base.lease(root / ".kv-cache-runtime.lock", blocking=True):
            evaluation_path = root / "budget-stop-evaluation-runtime.json"
            bind_reviewed_runtime_receipt(root / "mbpp-branch-quarantine-runtime.json", {
                "schema": "selection-switch-mbpp-branch-quarantine-runtime/v1",
                "switch_sha256": base.digest(root / "switch.json"),
                "runtime_code_hashes": current,
                "budget_stop_evaluation_runtime_sha256": (
                    base.digest(evaluation_path) if evaluation_path.exists() else None),
            ...
            if core.fingerprint(p["code_hashes"]) != core.fingerprint(current):
                base.bind(root / "curve-spawn-runtime.json", {
                    "schema": "selection-switch-curve-spawn-runtime/v1",
                    "switch_sha256": base.digest(root / "switch.json"),
                    "runtime_code_hashes": current,
                    "quarantine_runtime_sha256": base.digest(root / "mbpp-branch-quarantine-runtime.json"),
                    "change": "distinguish curve worker startup failures from peer point leases",
                    "cost_policy": "preserve all inputs, evaluations, checkpoints, costs, targets and budgets; no refunds",
                })
    return p
...
def curve_once(root, p, out, c, arm, suite, devices, env):
    ...
    for step in (*steps[1:], start):
        ...
        with contextlib.ExitStack() as stack:
            try:
                stack.enter_context(base.lease(target / ".point.lock"))
            except BlockingIOError:
                pending.append(step)
                continue
            # Worker startup may itself raise EAGAIN. Only acquisition above
            # means a peer owns this point; execution errors must reach retry.
            ...
            if commands:
                try:
                    ...
                    # Outer Switch queues also reserve BlockingIOError for
                    # task leases. Preserve the actual execution failure there.
                    raise RuntimeError(f"curve worker startup/allocation failed: {exc}") from exc
    if pending:
...
def mbpp_resume_blocked(p, directory):
    """Read-only checkpoint quarantine; caller must hold this branch's lease."""
    if p.get("dataset", "math500") != "mbpp":
        return False
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "switch_mbpp_storage_audit", HERE.parent.parent / "scripts/mbpp_storage_audit.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    if not module.policy_resume_blocked(directory):
        return False
    print(f"[WAIT] task={directory} checkpoint review required: prior training state has no "
          "complete checkpoint; skipping this branch without selection, training, or new charges", flush=True)
    return True
...
def smoke(root):
    ...
    with base.lease(out / "selection_reduced/.task.lock"):
        if mbpp_resume_blocked(p, out / "selection_reduced"):
            raise ValueError(f"MBPP smoke checkpoint review required: {out / 'selection_reduced'}")
        suite, protocol_value = core.read(child / "suite.json"), protocol(child)
        freeze_decisions(out, suite, protocol_value, env)
        runtime.run_arm(out, suite, protocol_value, "selection_reduced", devices, env)
...
def work(root, *, idle_timeout=600., only=None):
    ...
    quarantined, state_branches = set(), {}
    scoped_states = {(seed, step) for seed in (*rule.DEV_SEEDS, *rule.TEST_SEEDS)
                     if not only or seed in only["seeds"] for step in rule.STEPS}
    ...
    while True:
        ...
        for seed in (*rule.DEV_SEEDS, *rule.TEST_SEEDS):
            ...
            for step in rule.STEPS:
                ...
                try:
                    ...
                    state_branches[seed, step] = {
                        (seed, step, arm): out / arm for arm in protocol_value["arms"]
                        if not only or arm in only["arms"]}
                except BlockingIOError:
                    ...
                    try:
                        with base.lease(directory / ".task.lock"):
                            if branch_finished(p, directory):
                                continue
                            if mbpp_resume_blocked(p, directory):
                                attempted.add(key)
                                quarantined.add(key)
                                failures += 1
                                continue
        ...
        if progress:
            ...
            if not wait_for_peers(busy, last_progress=last_progress, idle_timeout=idle_timeout):
                break
    status(root)
    if (quarantined and failures == len(quarantined) and not busy
            and set(state_branches) == scoped_states
            and all((prefix_dir(root, seed) / f"prefix-{step}.json").exists()
                    for seed, step in scoped_states)
            and all(key in quarantined or branch_finished(p, directory)
                    for branches in state_branches.values() for key, directory in branches.items())):
        print(f"[queue] checkpoint review required for {len(quarantined)} branches; "
              "all other scoped work completed; releasing this worker without a retry loop", flush=True)
        return 80
