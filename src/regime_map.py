    ...
    *,
    ...
    for stratum, stratum_ids in strata.items():
        ...
        for policy, scores in policies.items():
            ...
            retention_lower = (
                selector_interval["retention"].get("lower_one_sided_95")
                if selector_interval
                else None
            )
            measurable = (
    ...
    *,
    ...
    for key, group in sorted(
        ...
        retained = [
            ...
            and (
                (
                    row["utility_retention_lower_one_sided_95"] is None
                    and row["utility_retention"] is not None
                    and row["utility_retention"] >= retention_threshold
                )
                or (
                    row["utility_retention_lower_one_sided_95"] is not None
                    and row["utility_retention_lower_one_sided_95"]
                    >= retention_threshold
                )
            )
        ]
        ...
        retention_lowers = [
            row["utility_retention_lower_one_sided_95"]
            for row in group
            if row["utility_retention_lower_one_sided_95"] is not None
        ]
        out.append(
            {
                "model": key[0],
                ...
                "median_retention_lower": (
                    statistics.median(retention_lowers) if retention_lowers else None
                ),
...
def render_report(summary: list[dict]) -> str:
    ...
    for row in summary:
        lines.append(
            ...
            f"[{_fmt(row['median_retention_lower'])}] | **{row['status']}** |"
        )
    return "\n".join(lines) + "\n"
