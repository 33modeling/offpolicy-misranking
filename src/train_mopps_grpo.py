"""Isolated online-selector integration of train_selection_gate_grpo at 7dc108a.

The budgeted learner is unchanged except prompt selection, reward feedback,
and posterior checkpoint evidence. Existing frozen switch runtimes are untouched.
"""
...
from mopps import Sampler, validate_policy_evidence
...
def gather_feedback(rewards, device, world_size):
    # The verifier returns CPU tensors; NCCL collectives require CUDA tensors.
    feedback = rewards.to(device=device).contiguous()
    groups = [torch.empty_like(feedback) for _ in range(world_size)]
    if world_size > 1:
        dist.all_gather(groups, feedback)
    else:
        groups[0] = feedback
    return [group.cpu().tolist() for group in groups]
...
def train(args: argparse.Namespace) -> None:
    ...
    selector_spec = json.loads(Path(args.selector_config).read_text())
    if (selector_spec["seed"] != args.seed or selector_spec["start_step"] != args.start_step
            or selector_spec["batch_size"] != world_size or selector_spec["responses"] != config.group_size):
        raise ValueError("online selector does not match the learner")
    sampler = Sampler(selector_spec)
    ...
    if (out_dir / "policy_train.json").is_file():
        try:
            ...
            if rank == 0:
                published = json.loads((out_dir / "policy_train.json").read_text())
                _atomic_json(out_dir / "budget_stop.json", {**published["training_budget"], "use_parent_policy": False})
    ...
    checkpoint_contract["online_selection"] = selector_spec
    ...
    if len(prompts) != selector_spec["pool_size"]:
        raise ValueError("online selector pool size changed")
    ...
    if world_size > 1:
        dist.barrier()
    history = [json.loads(line) for line in stats_path.read_text().splitlines() if line.strip()] if stats_path.exists() else []
    sampler.replay(history, completed_steps)
    ...
    try:
        for step in range(completed_steps, args.target_steps):
            ...
            proposal = sampler.begin(step + 1)
            prompt_index = proposal["selected"][rank]
            model.eval()
            ...
            evidence = sampler.finish(proposal, gather_feedback(rewards, next(model.parameters()).device, world_size))
        ...
        if rank == 0:
            ...
            _atomic_json(out_dir / "selector_state.json", sampler.state())
            manifest = {
                ...
                "online_selection": selector_spec,
                "selector_state_sha256": sha256_file(out_dir / "selector_state.json"),
                **parent_hashes,
            }
            if budget_deadline is not None:
