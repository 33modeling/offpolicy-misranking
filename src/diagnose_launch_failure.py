...
RULES: list[tuple[str, str, str]] = [
    ...
    (r"missing model\.safetensors-0000\d-of-0000\d\.safetensors \(no unique",
     ...
     "see [locate] lines below; re-upload the exact pinned file"),
    ...
    (r"file is not registered for the pinned model|model file missing: |model file (size|hash) mismatch",
     ...
     "prepare or upload the exact registered revision; do not bypass identity checks"),
    ...
    (r"waiting for shared snapshot preparation lock",
     ...
     "inspect the lock owner and its progress; do not terminate jobs by command-name pattern"),
    ...
    (r"unspecified launch failure|illegal memory access|CUBLAS_STATUS|device-side assert",
     "CUDA runtime failure; driver, kernel or code cause is not yet established",
     "preserve the traceback and GPU diagnostics before retrying"),
    ...
    (r"checkout is dirty|worktree is dirty",
     ...
     "git status; preserve and review local edits, then commit or use a separate clean checkout"),
