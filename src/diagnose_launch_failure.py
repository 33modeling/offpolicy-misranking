...
RULES: list[tuple[str, str, str]] = [
    ...
    (r"safetensors shard set incomplete|weight index contains no shards|neither a safetensors index",
     "index가 가리키는 shard 파일이 폴더에 없음 — 아래 [locate] 줄에 폴더 안 실제 파일이 나옴. *.safetensors가 하나도 없으면 가중치 미업로드",
     "가중치가 폴더에 있으면 git pull(552f625 이상) 후 재실행 — index를 실제 파일로 재구성함. 없으면 업로드/prepare"),
