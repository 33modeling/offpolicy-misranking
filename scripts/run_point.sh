...
# A completed GRPO policy and its immutable rollouts may be postprocessed by a
# newer analysis schema without pretending that they were generated by the new
# commit. Any experiment-setting mismatch still fails closed below.
ANALYSIS_UPGRADE=0
if [ "${OM_ALLOW_ANALYSIS_UPGRADE:-0}" = "1" ] \
    && [ -s "$OUT_ROOT/run_config.json" ]; then
  if "$PY" - "$OUT_ROOT/run_config.json" <<'PYEOF' >/dev/null 2>&1
import json
import subprocess
import sys
...
config = json.load(open(sys.argv[1]))
head = subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()
raise SystemExit(0 if config.get("git") != head else 1)
PYEOF
  then
    ANALYSIS_UPGRADE=1
  fi
fi
export ANALYSIS_UPGRADE
...
if lock.exists():
    ...
    if previous.get("digest") != config["digest"]:
        ...
        provenance = {"git", "git_diff_sha256", "git_status"}
        if os.environ.get("ANALYSIS_UPGRADE") != "1" or not set(changed) <= provenance:
            print(f"[config-abort] existing artifacts use a different run config: {changed}")
            sys.exit(2)
        print(
            "[analysis-upgrade] preserving source run config; changed provenance: "
            + ", ".join(changed)
        )
else:
    ...
    tmp.replace(lock)
if os.environ.get("ANALYSIS_UPGRADE") != "1":
...
if [ "$ANALYSIS_UPGRADE" = "1" ]; then
  ...
  PYTHONPATH="$PWD/src${PYTHONPATH:+:$PYTHONPATH}" \
    "$PY" - "$OUT_ROOT" "$DRIFT" "$NGPU" <<'PYEOF' || exit 1
import sys
from pathlib import Path

from recompute_oracle_scores import validate_analysis_upgrade

run, drift, world_size = Path(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3])
validate_analysis_upgrade(run, drift, world_size)
print("[analysis-upgrade] source rollouts, gradients, and policy validated")
PYEOF

  log "[analysis-upgrade] recomputing v2 R/A/B scores only; no training or generation"
  pids=()
  for i in $(seq 0 $((NGPU - 1))); do
    (
      CUDA_VISIBLE_DEVICES="${GPUS[$i]}" "$PY" src/experiment.py \
        --stage score-shard "${COMMON[@]}" "${POLICY_ARGS[@]}" --shard "$i:$NGPU" \
        >> "$LOGS/analysis-upgrade-score-$i.log" 2>&1
    ) &
    pids+=($!)
  done
  wait_all_stages "${pids[@]}" || exit 1
  CUDA_VISIBLE_DEVICES="${GPUS[0]}" "$PY" src/experiment.py \
    --stage merge-grads "${COMMON[@]}" >> "$LOGS/analysis-upgrade-merge.log" 2>&1 \
    || exit 1
  "$PY" src/recompute_oracle_scores.py "$OUT_ROOT" \
    >> "$LOGS/analysis-upgrade-report.log" 2>&1 || exit 1
  printf '%s\n' "analysis-upgraded $(date -Is)" > "$OUT_ROOT/DONE.tmp"
  mv "$OUT_ROOT/DONE.tmp" "$OUT_ROOT/DONE"
  log "[analysis-upgrade] v2 score/oracle/report complete"
  exit 0
fi
