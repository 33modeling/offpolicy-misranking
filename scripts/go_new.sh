...
POOL="$(om_hard_pool_path dapo-math "$MODEL_14B")" || exit 1
if [ ! -f "$POOL" ]; then
...
"$VENV_DIR/bin/python" src/make_hard_pool.py --validate "$POOL" \
  --model "$MODEL_14B" --dataset dapo-math || exit 1
[ -s "$POOL" ] || { echo "[abort] hard pool 0건: $POOL"; exit 1; }
