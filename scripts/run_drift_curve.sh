...
export MODEL_PATH="${MODEL_PATH:-$(field model-field path)}"
...
export REGIME_N_TRAIN_BY_DATASET="$(field experiment-field n_train_by_dataset 2>/dev/null | tr -d '{}"' | tr ',' ' ' | sed 's/: */=/g')"
export OM_ATTN=$(field experiment-field attn) OM_SKIP_HYBRID=1 OM_PROMPT_FORMAT="$FORMAT"
export OM_LORA_TARGETS="${OM_LORA_TARGETS:-$(field model-field lora_targets)}"
if [ -n "$PROFILE" ]; then
...
echo "[drift-curve] datasets=$DATASETS seeds=$SEEDS drifts=$DRIFTS root=$CURVE_ROOT"
