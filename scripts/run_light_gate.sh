...
MODE=${1:-run}
[ "$#" -eq 0 ] || shift
case "$MODE" in run|prepare|status|summarize|live|stop|plan|cpu|freeze) ;;
  *) echo 'usage: bash scripts/run_light_gate.sh [run|prepare|status|summarize|live|stop|plan|cpu|freeze]'; exit 2 ;;
esac
for option in "$@"; do
  case "$option" in --root|--root=*) echo '[abort] use LIGHT_GATE_ROOT to change the output root'; exit 2 ;; esac
done
...
export OM_WORK="$WORK"
PY=${LIGHT_GATE_PYTHON:-${VENV_DIR:-$WORK/.venv-cu126}/bin/python}
[ -x "$PY" ] || PY=python3
TAG=${OM_OLMO3_MODEL_TAG:-olmo3-1025-7b-base-rlzero-grpo-h100-v2}
MATRIX=${OM_OLMO3_ROOT:-$WORK/runs/$TAG}
export OUT_ROOT
OUT_ROOT=$(realpath -m "${LIGHT_GATE_ROOT:-$WORK/runs/selection-gate-light-v2}")
...
if [ "$MODE" = cpu ]; then
  ...
  CPU_PY=${LIGHT_GATE_CPU_PYTHON:-python3}
  "$CPU_PY" -m pytest -q tests/test_light_selection_gate.py
  exec "$CPU_PY" src/light_selection_gate.py audit
fi
if [ "$MODE" = plan ]; then
  printf '%s\n' '[v2] one whole-pool cache scan, one frozen screening decision; no per-epoch gate' \
    '[pair] random fixed 10% subset vs gated difficulty subset; equal allocated GPU-time caps' \
    '[default] exploratory OLMo MATH d100, seeds 0..4 = ten continuations; four GPUs per node' \
    '[measurement] actual difficulty-score correlation, mixed-group enrichment, score spread; <=1% allocation' \
    '[cost] measurement, failed attempts, scoring, training and checkpointing charged; evaluation separate' \
    '[scope] cached behavior-policy rewards are not current-policy utility; benchmark benefit is unproven' \
    '[held-out] --role test --rule frozen.json requires a frozen rule and disjoint source trajectories' \
    "[source] $MATRIX" "[output] $OUT_ROOT"
  exit 0
fi
if [ "$MODE" = freeze ]; then
  export CUDA_VISIBLE_DEVICES=""
  exec "$PY" src/light_selection_gate.py freeze --results "$OUT_ROOT/light_results.json" --out "$OUT_ROOT/frozen-rule.json"
fi
if [ "$MODE" = status ] || [ "$MODE" = summarize ]; then
  export CUDA_VISIBLE_DEVICES=""
  exec "$PY" src/light_selection_gate_gpu.py "$MODE" --root "$OUT_ROOT" "$@"
fi
if [ "$MODE" = live ]; then
  shopt -s nullglob
  declare -A FOLLOWED=()
...
if [ "$MODE" = stop ]; then
  exec "$PY" src/cleanup_run_processes.py --run-prefix "$OUT_ROOT" \
    --command-pattern "$OUT_ROOT" --command-pattern 'scripts/run_light_gate.sh' \
...
if [ "$MODE" = prepare ] || [ ! -f "$OUT_ROOT/suite.json" ] || [ ! -f "$OUT_ROOT/light_protocol.json" ] || [ "$#" -gt 0 ]; then
  ...
  "$PY" src/light_selection_gate_gpu.py prepare --root "$OUT_ROOT" --matrix "$MATRIX" \
...
trap '' HUP
"$PY" src/light_selection_gate_gpu.py run --root "$OUT_ROOT" 7>&- 8>&- &
CHILD=$!
