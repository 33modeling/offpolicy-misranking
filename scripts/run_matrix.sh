...
GRPO_LOGPROB_MICRO_BATCH_DEFAULT="${GRPO_LOGPROB_MICRO_BATCH:-1}"
GRPO_GRADIENT_CHECKPOINTING_DEFAULT="${GRPO_GRADIENT_CHECKPOINTING:-1}"
GRADIENT_MICRO_BATCH_DEFAULT="${GRADIENT_MICRO_BATCH:-1}"
...
run_point() {
  ...
  for try in $(seq 1 "$MAX_RETRIES"); do
    ...
    args=(env DATASET="$dataset" SEED="$seed" DRIFT="$drift" OUT_ROOT="$run"
      ...
      GRPO_LORA_RANK="$GRPO_LORA_RANK_DEFAULT"
      GRPO_LORA_ALPHA="$GRPO_LORA_ALPHA_DEFAULT"
      GRPO_LOGPROB_MICRO_BATCH="$GRPO_LOGPROB_MICRO_BATCH_DEFAULT"
      GRPO_GRADIENT_CHECKPOINTING="$GRPO_GRADIENT_CHECKPOINTING_DEFAULT"
      GRADIENT_MICRO_BATCH="$GRADIENT_MICRO_BATCH_DEFAULT"
