...
for name in ("torch", "transformers", "peft", "fla-core", "math-verify"):
    ...
    except Exception: print(f"{name}=없음")
try:
...
except Exception as e: print(f"qwen3_5 classes=없음 ({type(e).__name__})")
PYEOF
echo "--- 모델 ---"
...
for d in math500 mbpp; do
  ...
  [ -n "$f" ] && echo "$d: $f ($(wc -l < "$f") rows)" || echo "$d: $DATASETS_DIR/$d 아래 jsonl 없음 (업로드본은 내용으로 자동 인식됨 — run에서 확인)"
done
