...
if grep -q "gsm8k/s0 ✔" "$LOGDIR/retry-probe.log"; then
  ...
  SEEDS="${SEEDS_ALL:-0 1 2 3 4}" DATASETS="math500" N_TRAIN=400 N_VAL=100 bash scripts/go_v2.sh 2>&1 | tee -a "$LOGDIR/go_full.console.log"
  ...
  echo "   1) $LOGDIR/DIAGNOSIS.txt 를 md로 전달"
