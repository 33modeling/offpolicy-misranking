#!/usr/bin/env bash
# Finish only the saved On s1/t50 and Random s4/t100 evaluations.
# Existing experiment files are read-only; output goes to a separate root.
set -euo pipefail
MODE=${1:-run}
if [[ "$#" -gt 0 && "$1" != --* ]]; then shift; else MODE=run; fi
PAIR_FINISH_WORK="${OM_WORK:-/group-volume/${OM_USER:-minsoo3.kim}/offpolicy-misranking}"
export OM_WORK="$PAIR_FINISH_WORK"
SOURCE_PAIR_ROOT="${PAIR_ROOT:-$PAIR_FINISH_WORK/runs/selector-pair-v1}"
OUTPUT="${PAIR_FINAL_EVAL_ROOT:-$PAIR_FINISH_WORK/runs/selector-pair-final-eval-v1}"
export PAIR_FINISH_REPO="${PAIR_FINISH_REPO:-$PWD}"
[[ -f "$PAIR_FINISH_REPO/scripts/mbpp_budget_recovery.py" ]] || { echo '[abort] Run from the existing Pair checkout'; exit 2; }
cd "$PAIR_FINISH_REPO"
PY="${PAIR_PYTHON:-${VENV_DIR:-$PAIR_FINISH_WORK/.venv-cu126}/bin/python}"
[[ -x "$PY" ]] || { echo '[abort] Set PAIR_PYTHON to the existing experiment Python'; exit 2; }
export PYTHONPATH="$PAIR_FINISH_REPO/scripts:$PAIR_FINISH_REPO/src${PYTHONPATH:+:$PYTHONPATH}"
export PYTHONDONTWRITEBYTECODE=1 PYTHONUNBUFFERED=1
export OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1
export RAYON_NUM_THREADS=1 TOKENIZERS_PARALLELISM=false
case "$MODE" in
  plan|results) export CUDA_VISIBLE_DEVICES="" ;;
  run)
    export OUT_ROOT="$OUTPUT"
    source scripts/_e5_node.sh
    export E5_FORCE=0
    PAIR_ROOT="$OUTPUT"
    # Keep existing source tasks and locks intact, even if another owner is old.
    e5_recover_pair_gpu() { return 0; }
    e5_cleanup_lock_helpers() { return 0; }
    e5_acquire_node
    PAIR_ROOT="$SOURCE_PAIR_ROOT"
    if [[ -z "${CUDA_VISIBLE_DEVICES:-}" ]]; then
      PAIR_FINISH_VISIBLE=$(timeout 20 nvidia-smi --query-gpu=index --format=csv,noheader)
      export CUDA_VISIBLE_DEVICES="$(tr '\n' ',' <<< "$PAIR_FINISH_VISIBLE" | sed 's/,$//')"
    fi
    IFS=, read -r -a PAIR_FINISH_GPUS <<< "$CUDA_VISIBLE_DEVICES"
    [[ "${#PAIR_FINISH_GPUS[@]}" -eq 4 ]] || { echo '[abort] four allocated GPUs required'; exit 2; }
    PAIR_FINISH_MEMORY=$(timeout 20 nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits -i "$CUDA_VISIBLE_DEVICES")
    while read -r used; do
      [[ "$used" =~ ^[[:space:]]*[0-9]+[[:space:]]*$ ]] || { echo '[abort] invalid GPU status'; exit 2; }
      [[ "$used" -le 4000 ]] || { echo '[busy] GPUs occupied; nothing stopped'; exit 75; }
    done <<< "$PAIR_FINISH_MEMORY"
    ...
    ;;
  *) echo 'usage: bash finish_selector_pair_two.sh [run|plan|results] [--seed 1|4]'; exit 2 ;;
esac
PAIR_FINISH_TEMP=$(mktemp -d /tmp/pair-final-eval-runtime-XXXXXXXX)
trap 'rm -rf -- "$PAIR_FINISH_TEMP"' EXIT
cat > "$PAIR_FINISH_TEMP/selector_pair_finish_saved.py" <<'PAIR_FINISH_PYTHON'
#!/usr/bin/env python3
...
if __name__ == "__main__":
    main()
PAIR_FINISH_PYTHON
PAIR_FINISH_LOG=$(mktemp /tmp/pair-final-eval-XXXXXXXX.txt)
printf '[log] %s\n' "$PAIR_FINISH_LOG"
set +e
"$PY" -B "$PAIR_FINISH_TEMP/selector_pair_finish_saved.py" "$MODE" --root "$SOURCE_PAIR_ROOT" --output "$OUTPUT" "$@" 2>&1 | tee -- "$PAIR_FINISH_LOG"
PAIR_FINISH_STATUS=("${PIPESTATUS[@]}")
set -e
printf '[log] saved: %s\n' "$PAIR_FINISH_LOG"
[[ "${PAIR_FINISH_STATUS[1]}" -eq 0 ]] || exit "${PAIR_FINISH_STATUS[1]}"
exit "${PAIR_FINISH_STATUS[0]}"
