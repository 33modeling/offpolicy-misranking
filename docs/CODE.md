...
#### Q. 무엇을 기준으로 데이터를 선택하는가?

선택 단위는 개별 답변 토큰이 아니라 **학습 프롬프트(문제)**다. 각 프롬프트의
projected policy-gradient와 selection용 validation gradient의 cosine similarity를
점수로 사용하고 상위 10%를 선택한다.

```text
selection_score(x) = cosine(projected_prompt_gradient(x), validation_gradient)
```

현재 v4 설정에서는 GSM8K 512개 중 51개, MATH-500 400개 중 40개다. 프롬프트
`x`의 estimator별 gradient는 다음 형태다.

```text
g_e(x) = (1/K) sum_j sum_t A_j * w_e,j,t * grad log pi(a_j,t | h_j,t)
```

`A_j`는 rollout 정답 보상으로 만든 leave-one-out advantage이고, `w_e,j,t`가
`g00/g10/g01/g11`에 따라 달라지는 확률비 가중치다. 마지막 decoder block들의
parameter gradient를 고정 CountSketch로 4096차원에 투영한다. 따라서 여기서
embedding은 hidden-state 단어 임베딩이 아니라 **parameter-gradient embedding**이다.

#### Q. 보정은 무엇을 바꾸는가?

보정은 데이터 문장이나 정답을 수정하지 않는다. 옛 정책 `beta`의 rollout으로
프롬프트 gradient를 계산할 때 확률비를 다르게 곱해 **선택 점수와 순위**를 바꾼다.

```text
r_t = pi(a_t | h_t) / beta(a_t | h_t)
```

`r_t > 1`이면 현재 정책에서 그 토큰이 더 자연스럽고, `r_t < 1`이면 덜
자연스럽다는 뜻이다. 실제 구현은 완성된 비율을 `[1/10, 10]`으로 clipping한다.

#### Q. 실제 단어와 gradient embedding으로 g10/g01을 설명하면?

옛 정책 `beta`가 `The answer is 5`를 생성했고 토큰별 `pi/beta` 비율이
`[0.5, 1.0, 2.0, 4.0]`이라고 하자. 설명은 단어 단위지만 실제 코드는 tokenizer
토큰 단위다. 각 토큰의 gradient 방향을 `v_The`, `v_answer`, `v_is`, `v_5`로 쓰고
설명상 advantage를 1로 둔다.

| 토큰 | `The` | `answer` | `is` | `5` |
|---|---:|---:|---:|---:|
| `r_t` | 0.5 | 1.0 | 2.0 | 4.0 |
| `g10`: 앞에서 현재까지 누적 | 0.5 | 0.5 | 1.0 | 4.0 |
| `g01`: 현재부터 뒤까지 누적 | 4.0 | 8.0 | 8.0 | 4.0 |

```text
g10 = 0.5*v_The + 0.5*v_answer + 1.0*v_is + 4.0*v_5
g01 = 4.0*v_The + 8.0*v_answer + 8.0*v_is + 4.0*v_5
```

`g00`도 완전 무가중치가 아니라 현재 토큰 비율 `r_t`를 사용한다. `00`은 추가적인
prefix occupancy와 suffix outcome 보정을 모두 끈 **기준 셀**이다. `g11`은 전체
trajectory 비율을 모든 토큰에 적용한다.

#### Q. 앞누적 g10과 뒤누적 g01의 장단점은 무엇인가?

**`g10`: 응답 시작부터 현재 토큰까지 prefix 누적**

- 장점: 현재 정책이 그 토큰의 history까지 실제로 도달할 가능성을 반영한다. 정책
  drift가 문제 풀이의 초반 접근법이나 reasoning path를 바꾼 경우 `g00`보다 적절하다.
- 장점: `g11`처럼 모든 토큰에 전체 trajectory 비율을 곱하지 않아 full IS보다
  분산과 ESS 붕괴가 완화될 수 있다.
- 단점: 현재 토큰 뒤의 결론과 보상은 계속 `beta` continuation에 의존한다. 초반
  경로는 비슷하지만 현재 정책의 마무리가 달라진 경우 outcome bias가 남는다.
- 단점: 뒤쪽 토큰으로 갈수록 긴 prefix 곱을 사용하므로 분산과 clipping이 커진다.

**`g01`: 현재 토큰부터 응답 끝까지 suffix 누적**

- 장점: 현재 위치 이후의 reasoning과 최종 답변이 현재 정책에서 어떻게 바뀌는지
  반영한다. drift가 결론·후반 풀이·정답률을 주로 바꾼 경우 `g00`보다 적절하다.
- 장점: 역시 각 토큰에서 한쪽 축만 누적하므로 full trajectory IS보다 분산을 줄일
  가능성이 있다.
- 단점: 해당 토큰까지의 history는 여전히 `beta`가 만든 분포다. 현재 정책이 거의
  방문하지 않을 stale 경로도 선택 점수에 남는 occupancy bias가 있다.
- 단점: 앞쪽 토큰일수록 긴 suffix 곱을 사용하므로 초반 gradient의 분산과 clipping이
  커진다. 전체 답변이 끝나야 가중치를 계산할 수 있다.

따라서 둘 중 하나가 항상 우월하지 않다. **초반 경로 분포가 바뀌면 `g10`, 주어진
경로 이후의 결과 분포가 바뀌면 `g01`이 맞는 축**이다. 둘 다 한쪽 편향을 남기는
대신 `g11`의 고분산을 줄이는 bias-variance trade-off이며, 실제 우열은 drift 위치와
oracle reliability에 따라 달라진다.

#### Q. g11은 on-policy 데이터 선택인가?

아니다. `g11`도 `beta`가 생성한 stale rollout을 사용한다. 전체 trajectory
importance ratio로 현재 정책 목표를 추정하는 **fully corrected off-policy
selector**다. 진짜 fresh on-policy selector는 현재 정책 `pi`로 각 학습 프롬프트의
rollout을 새로 생성한 뒤 그 gradient로 순위를 만든다.

#### Q. 실험의 precision은 무엇인가?

각 estimator가 고른 상위 10%가 fresh-current-policy oracle의 상위 10%와 얼마나
겹치는지 측정한다. selection용 validation half와 oracle 평가용 validation half는
분리되어 있다.

```text
precision = |selected_by_estimator intersect selected_by_fresh_oracle| / k
```

따라서 precision은 정답률 자체가 아니라 **fresh oracle이 선택했을 프롬프트를 stale
estimator가 얼마나 재현했는가**다.
