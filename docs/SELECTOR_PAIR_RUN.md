...
이 인계 도구는 검토된 `a3ef650`의 **분기 단위 큐**와 상태 수정을 설치한다. 개발 최대 18개,
테스트 최대 24개 노드에서 독립 분기를 배정하며, 기존 개발·학습·테스트 고정
장벽과 실험 예산은 유지한다. `restart_selector_pair.sh`는 명시적인 전환 명령이다.
