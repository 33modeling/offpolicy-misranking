...
건드리지 않고 다른 상태를 찾는다. 분기 큐 전환은 기존 worker가 정상 종료한 뒤
새 checkout에서 동일 `PAIR_ROOT`로 재실행한다. 중단이 꼭 필요하면 해당 worker를
정상 종료하고 종료·잠금 해제를 확인한 뒤 이어서 실행한다. 잠금 파일 삭제, GPU 전체
