...
상태 큐에서 다른 노드를 기다릴 때는 잠금 대기와 다른 **무진행 180초** 기준을
사용한다. 검증된 완료 상태 수가 늘거나, 실제로 기다리는 busy 상태에서 최근
`running` heartbeat의 `updated` 값이 새로 증가해야 무진행 시간을 갱신한다.
다른 상태의 진행, `WAIT` 기록 갱신, 같은 과거 heartbeat를 다시 읽는 것은
기한을 연장하지 않는다. 정상적인 peer 진행이 계속 확인되면 상태 큐 대기는
