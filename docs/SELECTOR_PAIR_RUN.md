...
목표 미도달은 학습 결과이며 잠금 소유 상태가 아니다. 새 큐는 개발 분기가
모두 끝난 뒤 일부 목표가 미도달이어도 남은 고정 테스트 대조군을 실행한다.
개발 18개와 고정 테스트 18개는 기존 결과를 재사용하며, H 회귀 Adaptive 6개는
필요한 유한 H 값이 없으면 `BLOCKED`와 해당 개발 상태를 표시한다. 이것은
Adaptive 실행 성공이나 SR-GC 실행으로 처리되지 않는다. 전체 성공 종료도
보고하지 않으며, 미완료·다른 worker 소유 분기 및 실패 기록은 보존한다.
목표·예산·실측값을 바꾸거나 잠금 파일을 삭제하지 않는다.

인계 런타임은 기존 실험 소스 pin에 검증된 큐·상태 도우미 3개만 덧붙인다.
