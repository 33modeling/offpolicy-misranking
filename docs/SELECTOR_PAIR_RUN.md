...
큐는 개별 분기가 아니라 같은 seed/step의 **상태 단위**로 노드를 배정한다.
개발 9개 상태는 각 노드에서 두 selector를 순서대로 실행하고, 테스트 6개
상태는 각 노드에서 네 대조군을 순서대로 실행한다. 동시에 실제 상태 작업을
담당할 수 있는 노드는 개발에서 최대 9개, 테스트에서 최대 6개다. 개발 18개와
테스트 24개라는 기존 실험 수는 바뀌지 않는다. 추가 노드는 이미 배정된 작업을
복제하지 않고 다음 작업을 기다린다.
...
새 worker들은 실행 동안 root의 `.pair.lock`을 공유 잠금(SH)으로 유지하고,
상태별 잠금으로 서로 다른 상태를 배정받는다. 설정 준비와 런타임 영수증 발행,
...
시작하지 않고 `[WAIT]`를 출력하며 안전한 전환을 기다린다. 준비를 다른 노드가
진행 중인 경우에도 15초 간격으로 다시 확인한다. 잠금을 삭제하거나 기존
프로세스를 죽여 우회하지 않으며, 이미 worker가 있다는 이유로 참여 없이
정상 종료하지 않는다.
...
- `queue-workers/*.json`: 노드명, worker PID, 개발/테스트 단계, 담당 상태,
  RUN/WAIT/DONE, 최근 갱신 시각, 확인한 상태 수와 해당 worker의 실패 기록.
...
worker 기록의 `queue_wait_wall_seconds`는 상태 큐에서 측정한 대기시간이다.
...
한 worker는 같은 실행 pass에서 실패한 상태를 끝없이 다시 배정하지 않으며,
독립적인 다른 상태를 진행한다. 미완료 개발 결과로 predictor를 학습하거나
테스트 결정을 만들지 않는다. 노드 종료로 상태 잠금이 해제되면 다른 worker가
...
그대로 보존한다. 실행 중인 pair의 checkout을 변경하기 위한 자동 pull/restart
