...
- 테스트 seed 3, 4: on-policy, cached, random의 고정 대조군 18개는
  별도 버전의 실행 순서 변경 영수증을 검증한 worker에서 개발 분기와 병렬 실행한다.
  adaptive 6개는 개발 결과 전체 검증, predictor 학습, 테스트 6개 상태의
  모든 결정 저장을 마친 뒤에만 실행한다. 테스트 continuation은 총 24개다.
...
설정 생성 → 원본 검증·준비 → 개발 곡선과 고정 테스트 대조군 수집 → predictor
학습 → 테스트 결정 고정 → adaptive 학습·평가 → 보고서까지 진행한다. 목표 미도달 등 실험의 검증
조건을 충족하지 못하면 해당 단계에서 중단하며, 결과를 만들어 통과시키지 않는다.

큐는 같은 seed/step 안에서도 **독립된 분기 단위**로 노드를 배정한다.
개발 9개 상태의 두 selector는 최대 **18개 4-GPU 노드**, 고정 테스트 대조군도
최대 **18개 4-GPU 노드**에서 실행할 수 있다. 새 순서에서 결정 고정 전의
독립 분기 수 상한은 합계 **36개**이며, 실제로는 미완료·미점유 작업 수만큼만
배정한다. 개발이 완료되고 결정이 고정된 뒤에는 adaptive를 포함해 남은 테스트
분기 최대 **24개**가 대상이다. 전체 42개를 동시에 실행한다는 뜻은 아니다.
이는 큐의 배정 상한이며 클러스터 할당량이나 동일 배속을 보장하지 않는다.
공유 부모 평가 등은 잠금으로 직렬화한다.
...
테스트 6개 상태의 모든 결정이 저장·검증되기 전에는 adaptive 학습을 시작하지 않는다.
고정 테스트 대조군의 선행 실행은 아래 실행 순서 변경 영수증을 요구한다.
...
### 기존 Pair 실행 노드에서 안전하게 분기 큐로 인계

Adaptive는 이제 **SR-GC**다. 회귀 학습, 개발 H label, 목표 35% 도달을
기다리지 않는다. 현재 공통 parent 정책에서 순위 R로 고정한 on-policy 집합과
캐시 SR 집합을 독립 A/B gradient로 비교한다. 두 대비량의 평균 D가 음수이면
SR, 0 이상이면 on-policy를 선택한다. 판단 후 바로 그 집합을 별도 Adaptive
분기에서 학습한다. 기존 고정 Pair 분기와 데이터는 그대로 유지한다.

같은 parent의 검증된 R 점수가 있으면 재사용한다. A/B의 후보 gradient는 두
집합의 합집합에 대해서만 새로 측정하고, validation은 R/A/B의 분리된 문항을
쓴다. 회귀용 추가 학습은 없지만 새로운 A/B 측정에는 GPU 비용이 든다.
새 측정 비용과 재사용 R 점수의 기존 비용을 Adaptive 비용에 한 번 반영한다.
이 비용과 이후 보상, 목표 도달 여부는 D나 selector 선택의 입력이 아니다.

`pair-sr-gc-runtime.json`, `sr-gc/s{seed}-t{step}/decision.json`에 규칙과
입력 hash, D_A/D_B/D, 선택 집합 및 비용을 저장한다. 기존 H 회귀의 결정이나
Adaptive 학습이 이미 있으면 덮어쓰지 않고 인계를 거부한다. 완료된 고정 Pair는
재학습하지 않는다. 측정 실패는 명시적으로 보고하며 고정 대조군은 계속 실행한다.
이 실험은 각 Pair 시작 checkpoint에서 한 번 판단하는 실험이지, 한 학습 경로의
중간에서 여러 번 전환한 결과가 아니다. 목표 H는 보고용 분석으로만 유지한다.

인계 런타임은 기존 학습 소스 pin에 검증된 SR-GC·큐·상태 도우미만 덧붙인다.
실행 중 checkout과 기존 런타임 디렉터리를 수정하지 않는다. 이미 실행 중인
프로세스에는 자동 적용되지 않으므로 아래의 갱신된 재시작 파일로 인계한다.

SR-GC 보고서는 해당 격리 런타임에서 `python scripts/report_selector_pair_srgc.py
...
이 인계 도구는 코드에 고정된 검토 버전의 **분기 단위 큐**와 상태 수정을 설치한다.
현재 버전은 아래의 승인된 실행 순서 변경을 포함하며, 개발 분기와 고정 테스트
대조군을 병렬 배정한다. adaptive 결정 장벽과 실험 예산은 유지한다.
`restart_selector_pair.sh`는 명시적인 전환 명령이다.
일반 `run`이나 `status`가 정상 실행 중인 worker를 자동 종료하지는 않는다.

`git pull`은 이미 실행 중인 Python 프로세스의 배정 방식을 바꾸지 않는다.
기존 상태 EX 잠금 소유 worker가 종료되기 전에는 새 분기 worker도 그 상태를
건드리지 않고 다른 상태를 찾는다. 기존 worker가 자연스럽게 종료되면 위 인계
명령으로 동일 `PAIR_ROOT`에 참여할 수 있다. 실행 중 전환은 이 명령이 아래 검증을
통과한 로컬 worker에 정상 종료를 요청하고, 종료·잠금 해제를 확인한 뒤 이어서
실행하는 방식이다. 잠금 파일 삭제, GPU 전체
강제 초기화, root 복사·새 초기화는 하지 않는다. 저장된 체크포인트·최종 결과·곡선은
검증 후 재사용되며 이미 완료된 학습이나 평가를 다시 시작하지 않는다.

도구는 로컬 커널에서 실제 root 잠금 소유 PID를 확인하고, 같은 사용자·Pair root·
checkout 또는 검증된 고정 실행 경로의 controller인지 다시 검증한다. 구버전 EX
controller뿐 아니라 명시적 재시작 대상인 로컬 SH worker도 확인한다. EX·SH 모두
PID·mount namespace, cgroup, 명시된 GPU 할당 식별자가 현재 명령과 일치해야 한다.
재시작 shell에 GPU 설정이 없으면 검증된 기존 controller의 GPU 선택을 이어받아
재시작할 때 GPU 범위를 넓히지 않는다. 새 실행은 검증에 사용한 기존 controller의
실제 Python·가상환경 경로를 이어받으며 shell의 다른 Python으로 바꾸지 않는다.
동일 hostname만으로 소유권을 인정하지 않으며, 로컬 소유자가 여러 명이거나
소유권이 모호하면 종료하지 않는다. 재시작할 코드는 기존 checkout을 그대로
재실행하지 않고, 검토된 분기 실행 버전 `a3ef650bce3040af24b4db85938dd3d3ace241c2`을
...
따라서 구버전 checkout에 bash만 복사한 경우에도 구버전 상태 큐를 다시 띄우지
않는다. 원래 학습 경로의 소유권과 새 실행 경로의 frozen manifest·기존 runtime
영수증 호환성을 각각 확인한 다음 종료한다. 학습 중이면 trainer의 기존 검증기로
유효한 로컬 체크포인트를 확인한 뒤 pidfd로 확인된 controller에만 TERM을 전달한다. 기존
...
하지 않으며 별도 MBPP 작업이나 다른 노드의 Pair worker를 종료하지 않는다.
root가 공유 잠금을 허용하고 로컬 SH 소유자가 없으면 아무 controller도 종료하지
않고 같은 고정 분기 버전의 GPU·노드 입장 검사를 거쳐 참여한다. 로컬 SH 소유자가
있으면 위 소유권 검증과 정상 종료 절차를 거친다. Git 다운로드·별도 코드 검증
실패 시 현재 controller는 종료하지 않는다.
유지보수 시 Python 도우미를 수정하면 `python3 scripts/build_selector_pair_handoff.py`로
단일 파일을 재생성하고, 같은 명령의 `--check`로 원본과 일치하는지 검증한다.

### 미종료 비용 이벤트 확인

`unclosed cost event ... unknown cost cannot be treated as zero`는 단순히 노드가
부족하다는 뜻이 아니다. 해당 분기에 시작 기록은 있지만 검증된 종료 비용이 없어
다음 실행을 막는 오류다. `spent()`는 이미 저장된 atomic finish receipt를 자동으로
재반영한다. 종료 영수증 자체가 없는 경우에는 사진의 시각이나 마지막 heartbeat로
...
전체 크기는 1 MiB 이하이며 미종료 이벤트의 원문, ledger 해시, 종료 영수증 원문
또는 누락, 진행 기록, 정책·체크포인트 파일 목록을 담는다. 파일이 없거나 크기
제한으로 생략된 부분은 명시한다. 이는 진단이며 자동 복구나 완료 판정이 아니다.
실행 중인 작업 종료, 비용 수정, lock 생성·삭제, GPU 작업은 하지 않는다.
...
사용한다. 검증된 완료 상태·분기 수가 늘거나, 실제로 기다리는 busy 분기에서
`running` heartbeat의 event/경과시간/updated가 이전 관측과 달라져야 무진행 시간을
갱신한다. 관측한 노드의 monotonic 시계로 판단하므로 서버 간 시각 차이는 무관하다.
...
2026-09-21의 변경은 기존의 "어떤 테스트 continuation도 모든 결정 고정 전에는
실행하지 않는다"는 전역 실행 순서를 명시적으로 수정한다.
`pair-parallel-controls-runtime.json`의
`offpolicy-selector-pair/parallel-fixed-controls-v1` 영수증을 먼저 고정한 뒤,
테스트 seed 3/4의 step 25/50/100에서 다음 분기만 선행 허용한다.

- `on_policy/selection_full`: 6개.
- `cached/selection_full`: 6개.
- `on_policy/random_full`: 6개.

개발 작업을 우선 확인하고, 다른 개발 worker를 기다리는 동안 고정 대조군 하나를
실행한 뒤 개발 큐를 다시 확인한다. 개발과 관계없는 새 실행이나 무제한 재시도
루프를 만드는 방식이 아니다. `develop` 명령은 개발 전용이며, 선행 대조군 배정은
전체 `run` 경로에 적용된다.

`adaptive-on_policy`, `adaptive-cached`의 학습·scoring·평가를 선행 허용하는
변경은 아니다. 선택된 adaptive 6개는 기존의 검증된 `test-decisions.json`이
필요하며, 고정 대조군의 결과를 복사하거나 대신 채워 넣지 않는다.

기존 `pair.json`, 과학 코드 해시, branch contract, target, 예산, seed,
평가 조건, 저장된 결과와 비용 장부는 유지한다. 새 영수증은 원래 프로토콜과
실행 코드, 선행 허용 분기 목록에 연결된다. 영수증 없이 이미 시작된 테스트
작업을 이 변경으로 사후 승인하지 않는다. 이미 유효한 결정 장벽을 가진 실행은
기존의 테스트 실행 자격을 유지한다.

predictor fitting에는 기존 DEV 9개 상태만 사용한다. 테스트 대조군의 보상이나
곡선을 feature, target, threshold, selector 선택 또는 개발 라벨로 사용하지 않는다.
결정의 입력은 기존에 해시로 고정된 prefix 통계와 캐시뿐이다. 고정 대조군의
결과가 먼저 저장되더라도 adaptive의 미측정 결과나 비용 이득을 추정해서 채우지 않는다.

구버전 DEV worker의 현재 분기와 실제 잠금을 취소하거나 지우지 않는다.
구버전 freezer는 선행 테스트 artifact를 발견하면 원래의 엄격한 검사로 중단할 수
있다. 이 경우 새 실행 순서를 검증하는 worker가 fitting과 결정 고정을 담당해야 한다.
따라서 모든 구버전 freezer가 새 순서와 호환된다고 해석하면 안 된다.

이 변경을 적용할 때에는 정상 실행 중인 노드를 그대로 두고 **대기 중이거나 새로
할당한 노드**에서 최신 `restart_selector_pair.sh`를 실행한다. 최소 한 개의 새
worker가 이후 결정 고정까지 진행해야 한다. DEV가 끝난 뒤 fitting이나 freeze의
검증 자체가 실패하면 그 오류는 그대로 보고하며 대조군 실행으로 우회하지 않는다.

병렬 배정은 완료까지의 경과 시간을 바꾸는 운영 변경이다. 변경 전후 시간을
동일 스케줄의 반복 측정처럼 취급하지 않으며, 이미 소비된 GPU 시간과 실패·재시도
비용은 삭제하지 않는다. 큐 테스트의 성공과 실제 GPU 실행 완료는 별개다.
