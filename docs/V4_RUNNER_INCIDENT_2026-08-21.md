...
6. `src/prepare_run_path.py`가 배정된 smoke/GSM8K/MATH500 경로의
   `run_config.json.git`을 현재 HEAD와 비교한다.
7. Git이 다르면 전체 run 디렉터리를 `$OM_WORK/quarantine/v4/`로 원자 이동한다.
   삭제하지 않으며 active `runs/` namespace 밖으로 옮겨 harvest 혼입도 막는다.
