...
def checkout(tmp_path: Path, gpu_count: int = 4, runner_exit: int = 0) -> tuple[Path, dict[str, str]]:
...
def test_launcher_propagates_worker_failure(tmp_path: Path) -> None:
    root, env = checkout(tmp_path, runner_exit=7)
    result = subprocess.run(
        ["/bin/bash", "scripts/go_additional.sh"],
        cwd=root,
        env=env,
    ...
    assert "failed (rc=7)" in result.stdout
