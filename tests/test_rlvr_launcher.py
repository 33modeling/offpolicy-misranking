...
def checkout(tmp_path: Path, gpu_count: int = 4) -> tuple[Path, dict[str, str]]:
    ...
    fake_bin.mkdir()
    (root / "scripts/run_rlvr.sh").write_text((ROOT / "scripts/run_rlvr.sh").read_text())
    ...
    executable(
        ...
        "row={k:os.environ[k] for k in ('MODEL_PATH','REGIME_MODEL_TAG','REGIME_DATASETS','REGIME_SEEDS','REGIME_DRIFTS','GRPO_WORLD_SIZE','GRPO_GROUP_SIZE')}\n"
