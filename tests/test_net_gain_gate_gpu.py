...
def test_standalone_status_does_not_treat_allocation_as_eta(tmp_path):
    root = tmp_path / "suite"
    p = protocol()
    core.atomic_json(root / "net_protocol.json", p)
    ...
    assert "NOT ETA" in result.stdout and "no verified completion estimate" in result.stdout
    assert "estimated finish" not in result.stdout and "allocation floor" not in result.stdout
    assert "idle/debugging gaps are unmeasured, NOT zero" in result.stdout
    assert before == {path.relative_to(root): path.read_bytes() for path in root.rglob("*") if path.is_file()}


def test_status_does_not_charge_active_evaluation_as_remaining_training(tmp_path):
    root = tmp_path / "suite"
    core.atomic_json(root / "net_protocol.json", protocol())
    contract = root / "points/p0/contract.json"
    core.atomic_json(contract, {"config": {"seed": 0, "drift": 100}})
    core.atomic_json(root / "suite.json", {"schema": base.SCHEMA,
        "points": [{"name": "p0", "sha256": base.digest(contract)}], "budget_gpu_seconds": 14400.})
    arm = root / "points/p0/random_full"
    core.atomic_json(arm / "policy/budget_stop.json", {"stop_reason": "no_block_fits"})
    event = {"event_id": "evaluation", "ledger": "reporting", "phase": "evaluate",
             "gpus": 4, "gpu_type": "H100", "state": "started", "time": time.time() - 900}
    base.journal(arm / "cost.jsonl", event)
    core.atomic_json(arm / "progress.json", {**event, "state": "running", "updated": time.time(), "seconds": 900})
    result = subprocess.run(["bash", "scripts/status_net_gain_gate.sh"], cwd=gpu.HERE.parents[1],
        env={**os.environ, "NET_GATE_ROOT": str(root), "NET_GATE_NODES": "4", "NET_GATE_PYTHON": sys.executable},
        capture_output=True, text=True, timeout=10)
    assert result.returncode == 0, result.stderr
    assert "RUNNING 1" in result.stdout and "QUEUED 2" in result.stdout
    assert "0.50 h (NOT ETA)" in result.stdout
